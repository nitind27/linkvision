    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-2">
        {['offers', 'coupons'].map(t => (
          <button key={t} onClick={() => setTab(t)} className={`px-5 py-2 rounded-xl text-sm font-medium transition capitalize ${tab === t ? 'bg-blue-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
              ...
              <Card className="p-4 card-hover">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h3 className="font-semibold text-slate-700 dark:text-slate-200 text-sm">{offer.title}</h3>
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 p-2 bg-slate-50 dark:bg-slate-700/50 rounded-xl text-center">
                    <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">{offer.usageCount}</p>
                    <p className="text-xs text-slate-400">Used</p>
                  </div>
                </div>
                <p className="text-xs text-slate-400 mb-3">Expires: {formatDate(offer.endDate)}</p>
                <div className="flex gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {allCoupons.map((coupon, i) => (
            <motion.div key={coupon.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
              <Card className="p-4 card-hover border-dashed border-2 border-slate-200 dark:border-slate-700">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                  <div className="bg-slate-50 dark:bg-slate-700/50 p-2 rounded-lg">
                    ...
                    <p className="font-bold text-slate-700 dark:text-slate-200">{coupon.discountType === 'Percentage' ? `${coupon.discountValue}%` : formatCurrency(coupon.discountValue)}</p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-700/50 p-2 rounded-lg">
                    <p className="text-slate-400">Used</p>
                    <p className="font-bold text-slate-700 dark:text-slate-200">{coupon.usedCount}/{coupon.usageLimit}</p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-700/50 p-2 rounded-lg">
                    <p className="text-slate-400">Min Order</p>
                    <p className="font-bold text-slate-700 dark:text-slate-200">{formatCurrency(coupon.minOrder)}</p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-700/50 p-2 rounded-lg">
                    <p className="text-slate-400">Expires</p>
                    <p className="font-bold text-slate-700 dark:text-slate-200">{formatDate(coupon.expiryDate)}</p>
                  </div>
                </div>
                <div className="flex gap-2">

    ...
    <div className="space-y-5">
      ...
      <Card>
        <div className="flex gap-2 p-4 border-b border-slate-100 dark:border-slate-700">
          {['all', 'unread'].map(f => (
            <button key={f} onClick={() => setFilter(f)} className={`px-4 py-1.5 text-sm rounded-xl font-medium transition ${filter === f ? 'bg-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600'}`}>
        ...
        <div className="divide-y divide-slate-50 dark:divide-slate-700">
          {filtered.length === 0 ? (
            ...
            <motion.div key={n.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
              className={`flex items-start gap-3 p-4 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition cursor-pointer ${!n.read ? 'bg-blue-50/30 dark:bg-blue-900/5' : ''}`}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="font-semibold text-slate-700 dark:text-slate-200 text-sm">{n.title}</p>
              ...
              <button onClick={(e) => { e.stopPropagation(); remove(n.id); }} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-slate-400 hover:text-red-500 transition shrink-0">
                <Trash2 size={14} />
              </button>
            </motion.div>
          ))}

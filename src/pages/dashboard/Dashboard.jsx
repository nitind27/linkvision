...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2 shadow-xl text-xs">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[22px] font-bold text-gray-800 dark:text-white tracking-tight">Good morning, Admin 👋</h1>
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Here's what's happening at Link Vision today.</p>
        </div>
        <div className="hidden sm:flex items-center gap-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-2 text-sm">
          <span className="status-dot dot-green badge-live" />
          <span className="text-gray-600 dark:text-slate-300 font-medium text-[13px]">Live · All systems normal</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <Card className="xl:col-span-2 p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="font-semibold text-gray-700 dark:text-slate-200 text-[15px]">Revenue Overview</h3>
              <p className="text-[12px] text-gray-500 dark:text-slate-400 mt-0.5">Monthly revenue for 2024</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <select className="text-[12px] border border-gray-200 dark:border-slate-700 rounded-lg px-2.5 py-1.5 bg-white dark:bg-slate-800 text-gray-600 dark:text-slate-300 outline-none">
                <option>2024</option>
                <option>2023</option>
              </select>
            </div>
        ...
        {/* Category Donut */}
        <Card className="p-5">
          <h3 className="font-semibold text-gray-700 dark:text-slate-200 text-[15px] mb-4">Revenue Split</h3>
          <div className="flex justify-center">
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                ...
                <Tooltip formatter={v => formatCurrency(v)} contentStyle={{ borderRadius: '12px', border: '1px solid #e5e7eb', fontSize: 12 }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
          <div className="space-y-2 mt-2">
            {analyticsData.categoryRevenue.slice(0, 5).map((c, i) => (
              <div key={c.name} className="flex items-center gap-2.5">
                ...
                <span className="text-[12px] text-gray-500 dark:text-slate-400 flex-1 truncate">{c.name}</span>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <Card className="xl:col-span-2 p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-semibold text-gray-700 dark:text-slate-200 text-[15px]">Daily Orders</h3>
              <p className="text-[12px] text-gray-500 dark:text-slate-400 mt-0.5">Last 30 days</p>
            </div>
          </div>
        ...
        {/* Order Status */}
        <Card className="p-5">
          <h3 className="font-semibold text-gray-700 dark:text-slate-200 text-[15px] mb-4">Order Status</h3>
            ...
            {[
              ...
              <div key={s.label}>
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-2">
                    {s.icon}
                    <span className="text-[13px] text-gray-600 dark:text-slate-400">{s.label}</span>
                  </div>
          ...
          <div className="mt-5 pt-4 border-t border-gray-100 dark:border-slate-800">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[11px] text-gray-400 dark:text-slate-500">Inventory Alert</p>
                <div className="flex items-center gap-2 mt-1">
                  <AlertTriangle size={13} className="text-amber-500" />
                  <span className="text-[13px] font-semibold text-gray-700 dark:text-slate-200">{lowStock + outOfStock} items</span>
                  <span className="text-[11px] text-gray-400 dark:text-slate-500">need attention</span>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <Card className="xl:col-span-2">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-slate-800">
            <div>
              <h3 className="font-semibold text-gray-700 dark:text-slate-200 text-[15px]">Latest Orders</h3>
              <p className="text-[12px] text-gray-500 dark:text-slate-400 mt-0.5">Most recent transactions</p>
            </div>
              ...
              <tbody>
                {orders.slice(0, 7).map((order, i) => (
                  <motion.tr key={order.id}
                    ...
                    className="border-b border-gray-50 dark:border-slate-800/60 hover:bg-gray-50 dark:hover:bg-slate-800/40 transition-colors last:border-0"
                  >
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2.5">
                        <img src={order.customer.avatar} alt="" className="w-7 h-7 rounded-full object-cover ring-1 ring-gray-200 dark:ring-slate-700" />
                        <span className="text-[13px] font-medium text-gray-700 dark:text-slate-200 truncate max-w-[110px]">{order.customer.name}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3">
                      <span className="text-[13px] font-bold text-gray-700 dark:text-slate-200">{formatCurrency(order.total)}</span>
                    </td>
                    <td className="px-5 py-3">
                      <Badge status={order.deliveryStatus} />
                    </td>
                    <td className="px-5 py-3">
                      <span className="text-[12px] text-gray-500 dark:text-slate-400">{timeAgo(order.createdAt)}</span>
                    </td>
                  </motion.tr>
                ))}
              </tbody>
        ...
        <Card className="flex flex-col">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-slate-800">
            <div>
              <h3 className="font-semibold text-gray-700 dark:text-slate-200 text-[15px]">Recent Repairs</h3>
              <p className="text-[12px] text-gray-500 dark:text-slate-400 mt-0.5">{repairs.filter(r => r.status === 'In Progress').length} in progress</p>
            </div>
          ...
          <div className="flex-1 divide-y divide-gray-50 dark:divide-slate-800/60">
            {repairs.slice(0, 6).map((r, i) => (
              <motion.div key={r.id}
                initial={{ opacity: 0, x: -8 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: i * 0.05 }}
                className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 dark:hover:bg-slate-800/40 transition cursor-pointer"
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-semibold text-gray-700 dark:text-slate-200 truncate">{r.customer.name}</p>
                  <p className="text-[11.5px] text-gray-500 dark:text-slate-400 truncate">{r.issue}</p>
                </div>
                <div className="text-right shrink-0">
                  <Badge status={r.status} />
                  <p className="text-[11px] text-gray-500 dark:text-slate-400 mt-0.5">{formatCurrency(r.estimatedCost)}</p>
                </div>
              </motion.div>
            ))}
          </div>

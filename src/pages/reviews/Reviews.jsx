...
const StarRating = ({ rating }) => (
  <div className="flex gap-0.5">
    {Array.from({ length: 5 }, (_, i) => (
      <Star key={i} size={12} className={i < rating ? 'text-yellow-400 fill-yellow-400' : 'text-slate-300'} />
    ))}
  </div>
);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Average Rating', value: avgRating, icon: '⭐', color: 'bg-yellow-50 dark:bg-yellow-900/20' },
          { label: 'Approved', value: reviews.filter(r => r.status === 'Approved').length, icon: '✅', color: 'bg-emerald-50 dark:bg-emerald-900/20' },
          { label: 'Pending', value: reviews.filter(r => r.status === 'Pending').length, icon: '⏳', color: 'bg-yellow-50 dark:bg-yellow-900/20' },
          { label: 'Rejected', value: reviews.filter(r => r.status === 'Rejected').length, icon: '❌', color: 'bg-red-50 dark:bg-red-900/20' },
        ].map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
            className={`${s.color} rounded-2xl p-4 border border-slate-100 dark:border-slate-700`}>
      ...
      <Card>
        ...
        <div className="divide-y divide-slate-50 dark:divide-slate-700">
          {paginated.map((review, i) => (
            <motion.div key={review.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
              <div className="flex items-start gap-3">
                <img src={review.customer.avatar} alt="" className="w-9 h-9 rounded-full object-cover shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center flex-wrap gap-2 mb-1">
                    <p className="font-semibold text-slate-700 dark:text-slate-200 text-sm">{review.customer.name}</p>
                  ...
                  <p className="font-medium text-slate-700 dark:text-slate-200 text-sm mb-1">{review.title}</p>
                  ...
                  {review.reply && (
                    <div className="mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-xl border-l-2 border-blue-500">
                      ...
                      <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">{review.reply}</p>
                    </div>
                  )}
                </div>
